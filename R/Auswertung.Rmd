...
setwd("../Aerodatabox_API/AerodataboxDataExtraction")
rm(list = ls())
...
} else if (airport == "LOWW"){
  input <- "aerodata_flughafen_wien.csv"
} else if (airport == "EHAM"){
...
Weiters werden die Linienflüge aufgeteilt auf zwei Gruppen (über und unter 1500 km Distanz). Desweiteren wird definiert, dass ein Flug als verspätet gilt, wenn er mehr als `r m` Minuten (Median, der durchschnittlichen Verspätung) verspätet ist.
...
slices <- c(c,d)
lbls <- c("Verspätet", "Pünktlich")
pie3D(slices, labels = lbls, main="Langstreckenflüge - Verspätet / Pünktlich", explode = 0.1)
```
Insgesamt sind auf Basis dieser Annahmen `r round(anteil_kurz_late*100, 2)` Prozent der Kurzstreckenflüge verspätet. Von den Langstreckenflügen sind es `r round(anteil_lang_late*100, 2)` Prozent.
